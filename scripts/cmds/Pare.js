module.exports = {
  config: {
    ...
    countDown: 0,
    role: 0,
    shortDescription: "chat with pare.",
    longDescription: "Chat with pare",
    category: "ai",
    guide: {
      en: " '{pn} pare' "
    }
  ...
  onStart: async function ({ api, event, args }) {
    ...
    if (input.length < 2) {
      ...
      try {
        ...
        const response = await axios.get(`https://hercai.onrender.com/v3/hercai?question=${encodeURIComponent(prompt + input)}`);
